...
begin
  ...
  if ComboBox1.Text = 'DESENVOLVEDORA' then
  BEGIN
    Qry_BuscaMeusJogos.SQL.Add('AND UPPER (TRIM (C.NOME_DESENVOL)) LIKE ' + QuotedStr('%'+ UpperCase(Trim(Edit1.text))+'%'));
  END
  ELSE if ComboBox1.Text = 'NOME'  then
  BEGIN
    Qry_BuscaMeusJogos.SQL.Add('AND UPPER (TRIM (B.NOME_JOGO)) LIKE ' + QuotedStr('%'+ UpperCase(Trim(Edit1.text))+'%'));
  END
  ELSE IF ComboBox1.Text = 'CATEGORIA' then
  BEGIN
      Qry_BuscaMeusJogos.SQL.Add('AND UPPER (TRIM (D.NOME_CATEGORIA)) LIKE ' + QuotedStr('%'+ UpperCase(Trim(Edit1.text))+'%'));
  END
  ELSE if ComboBox1.Text = 'PLATAFORMA' then
  BEGIN
      Qry_BuscaMeusJogos.SQL.Add('AND UPPER (TRIM (E.NOME_PLAT)) LIKE ' + QuotedStr('%'+ UpperCase(Trim(Edit1.text))+'%'));
  END;
  ...
  DBGrid1.Columns.Items[2].Font.Style := [TFontStyle.fsBold];
  DBGrid1.Columns.Items[3].Font.Style := [TFontStyle.fsBold];
  DBGrid1.Columns.Items[4].Font.Style := [TFontStyle.fsBold];
  DBGrid1.Columns.Items[5].Font.Style := [TFontStyle.fsBold];
